...
def _build_additional_10_roles() -> Dict[str, Dict[str, Any]]:
    """Helper constructing 10 additional high-demand industry roles to provide 30 comprehensive role tracks."""
    tracks: Dict[str, Dict[str, Any]] = {}

    # 21. SITE RELIABILITY ENGINEER (SRE & CHAOS)
    tracks["site_reliability_engineer"] = {
        "track_id": "site_reliability_engineer", "title": "Site Reliability Engineer & Chaos Engineering (SRE)",
        "badge": "SRE & Resilience", "description": "SLIs/SLOs/SLAs error budgets, MTTR reduction, chaos engineering drills, and alert fatigue mitigation.",
        "skills": ["Kubernetes", "Prometheus", "Grafana", "Chaos Engineering", "Python", "SQL"],
        "questions": [
            AssessmentQuestion(id="sre_mcq_1", type="mcq", modality="mcq_fundamentals", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Error Budget & Burn Rate Dynamics", prompt="If a service has a 99.9% 30-day availability SLO and burns 10% of its monthly error budget in 1 hour, what is the 1-hour burn rate?", options=["A) 1x", "B) 14.4x", "C) 72x", "D) 100x"], correct_option=2, explanation="10% in 1 hour is 0.1 / (1 / 720 hours) = 72x burn rate.", time_limit_minutes=2),
            AssessmentQuestion(id="sre_mcq_2", type="mcq", modality="mcq_architecture", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Circuit Breakers & Bulkhead Isolation", prompt="What is the primary function of the Bulkhead pattern in distributed systems?", options=["A) Encrypting database connections.", "B) Isolating resources (like thread pools or connection pools) into discrete partitions so failure of one does not cascade.", "C) Compressing log files.", "D) Routing web traffic round-robin."], correct_option=1, explanation="Bulkheads partition resources to prevent cascading failure.", time_limit_minutes=2),
            AssessmentQuestion(id="sre_dsa_1", type="coding", modality="dsa_algorithms", section="coding", section_title="Section 2: Coding & DSA Challenges", title="SRE: Sliding Window Error Rate Calculator", prompt="Given list of request booleans (True=success, False=error) and window size K, return max error rate in any contiguous window of size K rounded to 2 decimals.", starter_code={"python": "def solution(requests, k):\n    if len(requests) < k: return 0.0\n    errors = sum(1 for x in requests[:k] if not x)\n    max_err = errors\n    for i in range(k, len(requests)):\n        if not requests[i]: errors += 1\n        if not requests[i - k]: errors -= 1\n        max_err = max(max_err, errors)\n    return round(max_err / k, 2)\n"}, test_cases=[{"input_data": [[True, False, False, True, False, False], 3], "expected_output": 0.67, "description": "Max 2 errors in window of 3 = 0.67"}], time_limit_minutes=8),
            AssessmentQuestion(id="sre_sql_1", type="sql", modality="sql_aggregation_analytics", section="sql", section_title="Section 2: Interactive SQL & Data Challenges", title="SQL: SRE Incident Downtime & MTTR Audit", prompt="Select service_name and sum of downtime_minutes for services where total downtime > 30 mins.", starter_code={"sql": "SELECT service_name, SUM(downtime_minutes) as total_downtime FROM incident_logs GROUP BY service_name HAVING SUM(downtime_minutes) > 30 ORDER BY total_downtime DESC;\n"}, test_cases=[{"description": "Downtime query", "setup_sql": "CREATE TABLE incident_logs (id INT, service_name TEXT, downtime_minutes INT); INSERT INTO incident_logs VALUES (1, 'auth-service', 25), (2, 'auth-service', 20), (3, 'search-service', 10);", "expected_output": [["auth-service", 45]]}], time_limit_minutes=8),
            AssessmentQuestion(id="sre_deb_1", type="debugging", modality="code_debugging", section="coding", section_title="Section 2: Code Debugging & Threat Defense", title="Debugging: Fix Exponential Backoff with Jitter", prompt="Implement exponential backoff calculation: min(max_backoff, base * (2 ** attempt)) and ensure delay never exceeds max_backoff.", starter_code={"python": "def solution(base, attempt, max_backoff):\n    calc = base * (2 ** attempt)\n    return min(max_backoff, calc)\n"}, test_cases=[{"input_data": [1, 5, 10], "expected_output": 10, "description": "Cap 32 at max 10"}], time_limit_minutes=8)
        ]
    }

    # 22. CYBERSECURITY ANALYST (SOC & THREAT INTEL)
    tracks["cybersecurity_analyst"] = {
        "track_id": "cybersecurity_analyst", "title": "Cybersecurity SOC Analyst & Incident Response",
        "badge": "SOC & Threat Intel", "description": "SIEM log correlation, MITRE ATT&CK tactics, malware containment, PCAP analysis, and network intrusion detection.",
        "skills": ["SIEM", "Splunk", "Wireshark", "MITRE ATT&CK", "Python", "SQL"],
        "questions": [
            AssessmentQuestion(id="secops_mcq_1", type="mcq", modality="security_audit", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Network Scan Recognition", prompt="What indicates a TCP SYN stealth scan in Wireshark packet captures?", options=["A) Frequent FIN-ACK exchanges.", "B) Repeated SYN packets sent to consecutive ports with immediate RST received, without completing the 3-way handshake.", "C) High UDP audio traffic.", "D) DNS TXT queries."], correct_option=1, explanation="Stealth SYN scanning sends SYN and resets before completing handshake.", time_limit_minutes=2),
            AssessmentQuestion(id="secops_mcq_2", type="mcq", modality="security_audit", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Lateral Movement Defense", prompt="Which architectural control best impedes an adversary's lateral movement within an enterprise network?", options=["A) Weak passwords.", "B) Zero Trust Network Access (ZTNA) with microsegmentation and least-privilege service identity.", "C) Storing credentials in plain text.", "D) Opening all internal firewall ports."], correct_option=1, explanation="Microsegmentation confines compromised hosts.", time_limit_minutes=2),
            AssessmentQuestion(id="secops_dsa_1", type="coding", modality="dsa_algorithms", section="coding", section_title="Section 2: Coding & DSA Challenges", title="Security: IP Address to 32-bit Integer Matcher", prompt="Convert standard IPv4 dot-decimal string to a 32-bit unsigned integer.", starter_code={"python": "def solution(ip_str):\n    parts = [int(p) for p in ip_str.split('.')]\n    return (parts[0] << 24) | (parts[1] << 16) | (parts[2] << 8) | parts[3]\n"}, test_cases=[{"input_data": ["192.168.1.1"], "expected_output": 3232235777, "description": "Convert 192.168.1.1"}], time_limit_minutes=8),
            AssessmentQuestion(id="secops_sql_1", type="sql", modality="sql_aggregation_analytics", section="sql", section_title="Section 2: Interactive SQL & Data Challenges", title="SQL: Detect Brute Force Authentication Anomalies", prompt="Find ip_address and count of failed logins where failed logins >= 3 in audit log.", starter_code={"sql": "SELECT ip_address, COUNT(*) as failed_attempts FROM auth_logs WHERE status = 'failed' GROUP BY ip_address HAVING COUNT(*) >= 3 ORDER BY failed_attempts DESC;\n"}, test_cases=[{"description": "Brute force attempts", "setup_sql": "CREATE TABLE auth_logs (id INT, ip_address TEXT, status TEXT); INSERT INTO auth_logs VALUES (1, '10.0.0.5', 'failed'), (2, '10.0.0.5', 'failed'), (3, '10.0.0.5', 'failed'), (4, '10.0.0.8', 'failed');", "expected_output": [["10.0.0.5", 3]]}], time_limit_minutes=8),
            AssessmentQuestion(id="secops_deb_1", type="debugging", modality="code_debugging", section="coding", section_title="Section 2: Code Debugging & Threat Defense", title="Debugging: Sanitize User Input to Prevent Directory Traversal", prompt="Strip directory traversal sequences '../' and return safe basename.", starter_code={"python": "import os\ndef solution(path_str):\n    clean = os.path.basename(path_str.replace('..', ''))\n    return clean.lstrip('/')\n"}, test_cases=[{"input_data": ["../../etc/passwd"], "expected_output": "passwd", "description": "Strip traversal"}], time_limit_minutes=8)
        ]
    }

    # 23. NETWORK INFRASTRUCTURE ENGINEER
    tracks["network_engineer"] = {
        "track_id": "network_engineer", "title": "Network Infrastructure & Systems Engineer",
        "badge": "Networking & BGP", "description": "BGP routing, VLANs, subnetting, TCP handshake dynamics, DNS resolution, and VPC peering.",
        "skills": ["BGP", "OSPF", "TCP/IP", "DNS", "Cisco/Juniper", "Python", "SQL"],
        "questions": [
            AssessmentQuestion(id="net_mcq_1", type="mcq", modality="mcq_fundamentals", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="TCP Connection Teardown", prompt="What is the purpose of the TIME_WAIT socket state in TCP teardown?", options=["A) Speeds up future connections.", "B) Ensures the remote endpoint received the final ACK and prevents old delayed duplicate packets from corrupting new connections.", "C) Frees RAM immediately.", "D) Re-encrypts TLS certificate."], correct_option=1, explanation="TIME_WAIT prevents duplicate segments from interfering with subsequent connections.", time_limit_minutes=2),
            AssessmentQuestion(id="net_mcq_2", type="mcq", modality="mcq_architecture", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="BGP Route Propagation", prompt="What mechanism does Border Gateway Protocol (BGP) use to prevent routing loops across Autonomous Systems (AS)?", options=["A) Split horizon only.", "B) AS-Path attribute: if a router sees its own AS number in the AS-Path, it discards the update.", "C) Hop count capped at 15.", "D) Disabling exterior gateway routing."], correct_option=1, explanation="BGP inspects AS_PATH to detect and discard loops.", time_limit_minutes=2),
            AssessmentQuestion(id="net_dsa_1", type="coding", modality="dsa_algorithms", section="coding", section_title="Section 2: Coding & DSA Challenges", title="Network: Subnet Broadcast Address Calculator", prompt="Given an IP address prefix (e.g. '192.168.1.') return the broadcast address ending in 255.", starter_code={"python": "def solution(prefix):\n    return f\"{prefix.rstrip('.')}.255\"\n"}, test_cases=[{"input_data": ["192.168.1."], "expected_output": "192.168.1.255", "description": "Format broadcast address"}], time_limit_minutes=8),
            AssessmentQuestion(id="net_sql_1", type="sql", modality="sql_aggregation_analytics", section="sql", section_title="Section 2: Interactive SQL & Data Challenges", title="SQL: Interface Packet Drop Telemetry", prompt="Select interface_name and packet_drops for interfaces where packet_drops > 100.", starter_code={"sql": "SELECT interface_name, SUM(dropped_packets) as total_drops FROM interface_stats GROUP BY interface_name HAVING SUM(dropped_packets) > 100 ORDER BY total_drops DESC;\n"}, test_cases=[{"description": "Interface drops", "setup_sql": "CREATE TABLE interface_stats (id INT, interface_name TEXT, dropped_packets INT); INSERT INTO interface_stats VALUES (1, 'eth0', 120), (2, 'eth0', 30), (3, 'eth1', 5);", "expected_output": [["eth0", 150]]}], time_limit_minutes=8),
            AssessmentQuestion(id="net_deb_1", type="debugging", modality="code_debugging", section="coding", section_title="Section 2: Code Debugging & Threat Defense", title="Debugging: Fix VLAN Tag Bit Mask Extraction", prompt="Given an 802.1Q 16-bit Tag Control Information integer, extract 12-bit VLAN ID (bits 0-11: value & 0x0FFF).", starter_code={"python": "def solution(tci):\n    return tci & 0x0FFF\n"}, test_cases=[{"input_data": [4196], "expected_output": 100, "description": "Extract VLAN 100 (4096 + 100)"}], time_limit_minutes=8)
        ]
    }

    # 24. MLOPS & MODEL SERVING ENGINEER
    tracks["aiops_mlops_engineer"] = {
        "track_id": "aiops_mlops_engineer", "title": "MLOps & AI Infrastructure Engineer",
        "badge": "MLOps & Model Serving", "description": "Triton Inference Server, model drift detection, feature stores, Kubeflow pipelines, and GPU utilization.",
        "skills": ["Kubeflow", "MLflow", "Triton", "Docker", "Python", "SQL"],
        "questions": [
            AssessmentQuestion(id="mlops_mcq_1", type="mcq", modality="mcq_fundamentals", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Model Drift Dynamics", prompt="What distinguishes Concept Drift from Covariate Shift?", options=["A) Covariate shift changes hardware; concept drift changes Python version.", "B) Covariate shift changes input feature distribution P(X), while concept drift changes the relationship between features and target P(Y|X).", "C) Concept drift only affects neural networks.", "D) There is no difference."], correct_option=1, explanation="Covariate shift: P(X) shifts. Concept drift: P(Y|X) underlying ground truth changes.", time_limit_minutes=2),
            AssessmentQuestion(id="mlops_mcq_2", type="mcq", modality="mcq_architecture", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Inference Serving Optimization", prompt="Why does dynamic batching in Triton Inference Server increase GPU throughput?", options=["A) It converts FP32 to INT8 automatically.", "B) It aggregates individual requests arriving within a short delay window into a single batch, maximizing tensor core saturation.", "C) It turns off GPU cooling.", "D) It executes models on CPU."], correct_option=1, explanation="Dynamic batching amortizes memory transfer overhead across multiple inputs.", time_limit_minutes=2),
            AssessmentQuestion(id="mlops_dsa_1", type="coding", modality="dsa_algorithms", section="coding", section_title="Section 2: Coding & DSA Challenges", title="MLOps: Simple Feature Normalization Cache", prompt="Given a list of float numbers, return min-max normalized list rounded to 2 decimal places.", starter_code={"python": "def solution(nums):\n    mn, mx = min(nums), max(nums)\n    if mx == mn: return [0.0] * len(nums)\n    return [round((x - mn) / (mx - mn), 2) for x in nums]\n"}, test_cases=[{"input_data": [[10, 20, 30]], "expected_output": [0.0, 0.5, 1.0], "description": "Min-max normalization"}], time_limit_minutes=8),
            AssessmentQuestion(id="mlops_sql_1", type="sql", modality="sql_aggregation_analytics", section="sql", section_title="Section 2: Interactive SQL & Data Challenges", title="SQL: High Latency Inference Model Audit", prompt="Select model_name and avg latency for models where avg latency > 50ms.", starter_code={"sql": "SELECT model_name, ROUND(AVG(latency_ms), 1) as avg_latency FROM model_inference_logs GROUP BY model_name HAVING AVG(latency_ms) > 50 ORDER BY avg_latency DESC;\n"}, test_cases=[{"description": "High latency models", "setup_sql": "CREATE TABLE model_inference_logs (id INT, model_name TEXT, latency_ms INT); INSERT INTO model_inference_logs VALUES (1, 'llm-7b', 80), (2, 'llm-7b', 60), (3, 'bert-tiny', 12);", "expected_output": [["llm-7b", 70.0]]}], time_limit_minutes=8),
            AssessmentQuestion(id="mlops_deb_1", type="debugging", modality="code_debugging", section="coding", section_title="Section 2: Code Debugging & Threat Defense", title="Debugging: Fix Model Metric Drift Threshold Alert", prompt="Return True if current accuracy dropped below baseline by more than threshold.", starter_code={"python": "def solution(baseline, current, threshold):\n    return (baseline - current) > threshold\n"}, test_cases=[{"input_data": [0.95, 0.88, 0.05], "expected_output": True, "description": "0.07 drop > 0.05 threshold"}], time_limit_minutes=8)
        ]
    }

    # 25. BIG DATA ARCHITECT & DISTRIBUTED STORAGE
    tracks["big_data_architect"] = {
        "track_id": "big_data_architect", "title": "Big Data Architect & Distributed Storage",
        "badge": "Spark & Lakehouse", "description": "Apache Spark execution DAGs, Kafka partitions, Apache Iceberg ACID lakehouses, and Parquet columnar storage.",
        "skills": ["Apache Spark", "Kafka", "Iceberg", "Delta Lake", "Hadoop", "SQL"],
        "questions": [
            AssessmentQuestion(id="bda_mcq_1", type="mcq", modality="mcq_fundamentals", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Spark Data Skew & Salting", prompt="How does key salting resolve severe data skew in Apache Spark distributed shuffle joins?", options=["A) Compresses the dataset with gzip.", "B) Appends a random integer suffix to skewed join keys to distribute identical keys evenly across multiple reducer partitions.", "C) Drops skewed rows.", "D) Increases driver RAM."], correct_option=1, explanation="Salting divides popular keys into multiple partitions.", time_limit_minutes=2),
            AssessmentQuestion(id="bda_mcq_2", type="mcq", modality="mcq_architecture", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Lakehouse ACID Storage Formats", prompt="How do table formats like Apache Iceberg and Delta Lake guarantee ACID transactions on object storage (S3/GCS)?", options=["A) Locking the entire cloud bucket.", "B) Using immutable Parquet files with snapshot metadata trees and atomic commit logs.", "C) Replacing S3 with MySQL.", "D) Storing all data in RAM."], correct_option=1, explanation="Iceberg uses snapshot manifest files and atomic pointer swaps.", time_limit_minutes=2),
            AssessmentQuestion(id="bda_dsa_1", type="coding", modality="dsa_algorithms", section="coding", section_title="Section 2: Coding & DSA Challenges", title="Big Data: Key Salting Hash Distributor", prompt="Given a key string and salt modulus N, return list of salted keys f'{key}_{i}' for i from 0 to N-1.", starter_code={"python": "def solution(key, n):\n    return [f\"{key}_{i}\" for i in range(n)]\n"}, test_cases=[{"input_data": ["user_click", 3], "expected_output": ["user_click_0", "user_click_1", "user_click_2"], "description": "Generate 3 salted keys"}], time_limit_minutes=8),
            AssessmentQuestion(id="bda_sql_1", type="sql", modality="sql_aggregation_analytics", section="sql", section_title="Section 2: Interactive SQL & Data Challenges", title="SQL: Large Partition Data Size Monitor", prompt="Select table_name and sum of partition_mb where total size > 1000 MB.", starter_code={"sql": "SELECT table_name, SUM(partition_mb) as total_mb FROM lakehouse_partitions GROUP BY table_name HAVING SUM(partition_mb) > 1000 ORDER BY total_mb DESC;\n"}, test_cases=[{"description": "Large partitions", "setup_sql": "CREATE TABLE lakehouse_partitions (id INT, table_name TEXT, partition_mb INT); INSERT INTO lakehouse_partitions VALUES (1, 'events', 800), (2, 'events', 500), (3, 'users', 200);", "expected_output": [["events", 1300]]}], time_limit_minutes=8),
            AssessmentQuestion(id="bda_deb_1", type="debugging", modality="code_debugging", section="coding", section_title="Section 2: Code Debugging & Threat Defense", title="Debugging: Fix Stream Watermark Delay Calculation", prompt="Given current event timestamp and max allowed lateness seconds, return watermark threshold timestamp.", starter_code={"python": "def solution(current_event_ts, max_lateness):\n    return current_event_ts - max_lateness\n"}, test_cases=[{"input_data": [1000, 30], "expected_output": 970, "description": "Watermark is 1000 - 30 = 970"}], time_limit_minutes=8)
        ]
    }

    # 26. CRM & ENTERPRISE SOLUTIONS DEVELOPER
    tracks["crm_enterprise_developer"] = {
        "track_id": "crm_enterprise_developer", "title": "Enterprise Systems & CRM Solutions Engineer",
        "badge": "Enterprise & Workflows", "description": "Salesforce Apex governor limits, SOAP/REST enterprise integration, SOQL optimization, and event-driven automation.",
        "skills": ["Salesforce/Apex", "SOQL", "ServiceNow", "Enterprise Integration", "Python", "SQL"],
        "questions": [
            AssessmentQuestion(id="crm_mcq_1", type="mcq", modality="mcq_fundamentals", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Salesforce Governor Limits", prompt="Why does Salesforce enforce strict SOQL governor limits (e.g. max 100 SOQL queries per transaction)?", options=["A) To conserve hard drive space.", "B) In a multi-tenant cloud architecture, governor limits prevent runaway code in one tenant from monopolizing shared resources.", "C) SOQL cannot query more than 100 rows.", "D) To force developers to use Java."], correct_option=1, explanation="Multi-tenant fairness requires limiting resource consumption per transaction.", time_limit_minutes=2),
            AssessmentQuestion(id="crm_mcq_2", type="mcq", modality="mcq_architecture", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Enterprise Bulkification", prompt="What is 'bulkification' in enterprise CRM development?", options=["A) Increasing database font size.", "B) Designing code and triggers to process collections of records (up to 200) simultaneously rather than individual records.", "C) Exporting data to CSV.", "D) Deleting duplicate accounts."], correct_option=1, explanation="Bulkified triggers process batches to adhere to governor limits.", time_limit_minutes=2),
            AssessmentQuestion(id="crm_dsa_1", type="coding", modality="dsa_algorithms", section="coding", section_title="Section 2: Coding & DSA Challenges", title="Enterprise: Record Deduplication Key Generator", prompt="Given email and phone, return normalized lowercase key with phone stripped of non-digits: f'{clean_email}:{digits_only}'.", starter_code={"python": "import re\ndef solution(email, phone):\n    clean_email = email.strip().lower()\n    clean_phone = re.sub(r'\\D', '', phone)\n    return f\"{clean_email}:{clean_phone}\"\n"}, test_cases=[{"input_data": ["John.Doe@Corp.com", "(555) 123-4567"], "expected_output": "john.doe@corp.com:5551234567", "description": "Clean deduplication key"}], time_limit_minutes=8),
            AssessmentQuestion(id="crm_sql_1", type="sql", modality="sql_aggregation_analytics", section="sql", section_title="Section 2: Interactive SQL & Data Challenges", title="SQL: High-Value Enterprise Opportunity Funnel", prompt="Select stage_name and sum of amount for stages with total amount >= 100000.", starter_code={"sql": "SELECT stage_name, SUM(amount) as total_val FROM crm_deals GROUP BY stage_name HAVING SUM(amount) >= 100000 ORDER BY total_val DESC;\n"}, test_cases=[{"description": "Opportunity pipeline", "setup_sql": "CREATE TABLE crm_deals (id INT, stage_name TEXT, amount INT); INSERT INTO crm_deals VALUES (1, 'Closed Won', 80000), (2, 'Closed Won', 40000), (3, 'Prospecting', 15000);", "expected_output": [["Closed Won", 120000]]}], time_limit_minutes=8),
            AssessmentQuestion(id="crm_deb_1", type="debugging", modality="code_debugging", section="coding", section_title="Section 2: Code Debugging & Threat Defense", title="Debugging: Fix Trigger Recursion Guard", prompt="Return False if execution count exceeds max allowable recursion depth.", starter_code={"python": "def solution(depth, max_depth):\n    return depth < max_depth\n"}, test_cases=[{"input_data": [5, 5], "expected_output": False, "description": "Block when depth reaches max"}], time_limit_minutes=8)
        ]
    }

    # 27. AR/VR & SPATIAL COMPUTING ENGINEER
    tracks["ar_vr_engineer"] = {
        "track_id": "ar_vr_engineer", "title": "AR/VR & Spatial Computing Engineer",
        "badge": "Spatial & WebXR", "description": "Spatial anchor tracking, quaternion rotation, WebXR frame pacing, shader optimizations, and 3D bounding volumes.",
        "skills": ["Unity", "Unreal", "WebXR", "Linear Algebra", "C#", "C++", "Python"],
        "questions": [
            AssessmentQuestion(id="ar_mcq_1", type="mcq", modality="mcq_fundamentals", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Quaternions vs Euler Angles", prompt="Why are quaternions preferred over Euler angles for representing 3D rotations in AR/VR engines?", options=["A) Quaternions use less memory.", "B) Quaternions eliminate gimbal lock and provide smooth spherical linear interpolation (SLERP).", "C) Quaternions only work in 2D.", "D) Euler angles cannot rotate along the Z-axis."], correct_option=1, explanation="Quaternions avoid singularities (gimbal lock) during composite rotations.", time_limit_minutes=2),
            AssessmentQuestion(id="ar_mcq_2", type="mcq", modality="mcq_architecture", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Foveated Rendering", prompt="What is the architectural purpose of Eye-Tracked Foveated Rendering in modern VR headsets?", options=["A) Increasing battery heat.", "B) Rendering only the focal area at full resolution while blurring peripheral vision, drastically reducing GPU shading workload.", "C) Turning off audio.", "D) Capturing photos."], correct_option=1, explanation="Renders high resolution only where the user's fovea is directed.", time_limit_minutes=2),
            AssessmentQuestion(id="ar_dsa_1", type="coding", modality="dsa_algorithms", section="coding", section_title="Section 2: Coding & DSA Challenges", title="Spatial: 3D Axis-Aligned Bounding Box (AABB) Overlap", prompt="Detect if two 1D intervals [min1, max1] and [min2, max2] overlap.", starter_code={"python": "def solution(min1, max1, min2, max2):\n    return max(min1, min2) <= min(max1, max2)\n"}, test_cases=[{"input_data": [0, 5, 4, 10], "expected_output": True, "description": "Interval overlap"}], time_limit_minutes=8),
            AssessmentQuestion(id="ar_sql_1", type="sql", modality="sql_aggregation_analytics", section="sql", section_title="Section 2: Interactive SQL & Data Challenges", title="SQL: VR Headset Frame Rate Drop Telemetry", prompt="Select user_id and count of dropped frame events where frame_rate < 72.", starter_code={"sql": "SELECT user_id, COUNT(*) as drop_count FROM vr_session_telemetry WHERE frame_rate < 72 GROUP BY user_id HAVING COUNT(*) >= 2 ORDER BY drop_count DESC;\n"}, test_cases=[{"description": "Frame rate drops", "setup_sql": "CREATE TABLE vr_session_telemetry (id INT, user_id TEXT, frame_rate INT); INSERT INTO vr_session_telemetry VALUES (1, 'u1', 60), (2, 'u1', 65), (3, 'u2', 90);", "expected_output": [["u1", 2]]}], time_limit_minutes=8),
            AssessmentQuestion(id="ar_deb_1", type="debugging", modality="code_debugging", section="coding", section_title="Section 2: Code Debugging & Threat Defense", title="Debugging: Fix Degrees to Radians Conversion", prompt="Convert angle in degrees to radians: degrees * (math.pi / 180). Return rounded to 4 decimals.", starter_code={"python": "import math\ndef solution(deg):\n    return round(deg * (math.pi / 180.0), 4)\n"}, test_cases=[{"input_data": [180], "expected_output": 3.1416, "description": "180 degrees to pi"}], time_limit_minutes=8)
        ]
    }

    # 28. FINTECH & QUANTITATIVE SYSTEMS DEVELOPER
    tracks["fintech_quant_developer"] = {
        "track_id": "fintech_quant_developer", "title": "FinTech & Quantitative Systems Developer",
        "badge": "High-Freq & Trading", "description": "Order book limit orders, FIX protocol messaging, latency profiling, VWAP calculation, and risk limits.",
        "skills": ["C++", "Python", "FIX Protocol", "Algorithms", "Financial Math", "SQL"],
        "questions": [
            AssessmentQuestion(id="fin_mcq_1", type="mcq", modality="mcq_fundamentals", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Order Matching Priority", prompt="Under Price-Time Priority (FIFO) matching, what determines order precedence when two buy orders share the same bid price?", options=["A) Larger order size.", "B) The order with the earlier arrival timestamp executes first.", "C) Alphabetical broker name.", "D) Random selection."], correct_option=1, explanation="FIFO matches at identical price levels based on arrival timestamp.", time_limit_minutes=2),
            AssessmentQuestion(id="fin_mcq_2", type="mcq", modality="mcq_architecture", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Kernel Bypass in Ultra-Low Latency Trading", prompt="Why do High-Frequency Trading (HFT) platforms use kernel-bypass networking (e.g. Solarflare EF_VI or DPDK)?", options=["A) To encrypt internet traffic.", "B) To route packets directly from the NIC ring buffer to user-space memory, bypassing OS socket buffers and context switch overhead.", "C) To slow down packet processing.", "D) To support WiFi."], correct_option=1, explanation="Eliminates Linux kernel TCP/IP stack overhead, reducing tick-to-trade latency to sub-microsecond.", time_limit_minutes=2),
            AssessmentQuestion(id="fin_dsa_1", type="coding", modality="dsa_algorithms", section="coding", section_title="Section 2: Coding & DSA Challenges", title="FinTech: Volume Weighted Average Price (VWAP)", prompt="Given list of [price, volume] trades, calculate VWAP: sum(price * volume) / sum(volume), rounded to 2 decimals.", starter_code={"python": "def solution(trades):\n    total_val = sum(p * v for p, v in trades)\n    total_vol = sum(v for _, v in trades)\n    if total_vol == 0: return 0.0\n    return round(total_val / total_vol, 2)\n"}, test_cases=[{"input_data": [[[100.0, 10], [105.0, 20]]], "expected_output": 103.33, "description": "VWAP calculation"}], time_limit_minutes=8),
            AssessmentQuestion(id="fin_sql_1", type="sql", modality="sql_aggregation_analytics", section="sql", section_title="Section 2: Interactive SQL & Data Challenges", title="SQL: Large Trade Volume Alert", prompt="Select ticker and sum of volume for trades where sum of volume > 50000.", starter_code={"sql": "SELECT ticker, SUM(shares) as total_shares FROM trade_executions GROUP BY ticker HAVING SUM(shares) > 50000 ORDER BY total_shares DESC;\n"}, test_cases=[{"description": "High volume trades", "setup_sql": "CREATE TABLE trade_executions (id INT, ticker TEXT, shares INT); INSERT INTO trade_executions VALUES (1, 'AAPL', 30000), (2, 'AAPL', 30000), (3, 'GOOG', 1000);", "expected_output": [["AAPL", 60000]]}], time_limit_minutes=8),
            AssessmentQuestion(id="fin_deb_1", type="debugging", modality="code_debugging", section="coding", section_title="Section 2: Code Debugging & Threat Defense", title="Debugging: Fix Basis Points (bps) Calculation", prompt="Convert decimal difference to basis points (1 bp = 0.0001, so bps = diff * 10000). Return integer.", starter_code={"python": "def solution(p1, p2):\n    diff = abs(p1 - p2)\n    return int(round(diff * 10000))\n"}, test_cases=[{"input_data": [1.0025, 1.0000], "expected_output": 25, "description": "25 bps difference"}], time_limit_minutes=8)
        ]
    }

    # 29. BIOINFORMATICS & COMPUTATIONAL GENOMICS
    tracks["bioinformatics_engineer"] = {
        "track_id": "bioinformatics_engineer", "title": "Bioinformatics & Computational Genomics Engineer",
        "badge": "Genomics & BioPython", "description": "FASTQ/VCF variant calling, Smith-Waterman sequence alignment, k-mer counting, and gene expression profiling.",
        "skills": ["Python", "Bioinformatics", "Genomics", "VCF/BAM", "Algorithms", "SQL"],
        "questions": [
            AssessmentQuestion(id="bio_mcq_1", type="mcq", modality="mcq_fundamentals", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="FASTQ Phred Quality Scores", prompt="In high-throughput sequencing FASTQ format, what does a Phred quality score of Q30 represent?", options=["A) 30% sequencing error rate.", "B) 99.9% base call accuracy (1 in 1,000 error probability).", "C) 30 amino acids translated.", "D) Sequence length is 30 base pairs."], correct_option=1, explanation="Q = -10 * log10(P). Q30 = 1 in 1000 probability of error = 99.9% accuracy.", time_limit_minutes=2),
            AssessmentQuestion(id="bio_mcq_2", type="mcq", modality="mcq_architecture", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Burrows-Wheeler Transform in Read Alignment", prompt="Why is the Burrows-Wheeler Transform (BWT) with FM-indexing used in alignment tools like BWA and Bowtie?", options=["A) It compresses DNA sequences lossily.", "B) It allows searching the entire 3-billion base-pair human reference genome in memory with linear time complexity.", "C) It translates DNA directly to 3D protein folds.", "D) It deletes PCR duplicates."], correct_option=1, explanation="BWT enables exact pattern matching across gigantic reference genomes in small RAM footprints.", time_limit_minutes=2),
            AssessmentQuestion(id="bio_dsa_1", type="coding", modality="dsa_algorithms", section="coding", section_title="Section 2: Coding & DSA Challenges", title="Bioinformatics: DNA Sequence Reverse Complement", prompt="Given DNA sequence string of 'A', 'C', 'G', 'T', return its reverse complement ('A'<->'T', 'C'<->'G').", starter_code={"python": "def solution(seq):\n    trans = str.maketrans('ACGTacgt', 'TGCAtgca')\n    return seq.translate(trans)[::-1]\n"}, test_cases=[{"input_data": ["ATGC"], "expected_output": "GCAT", "description": "Reverse complement of ATGC"}], time_limit_minutes=8),
            AssessmentQuestion(id="bio_sql_1", type="sql", modality="sql_aggregation_analytics", section="sql", section_title="Section 2: Interactive SQL & Data Challenges", title="SQL: Pathogenic Genomic Variant Frequency", prompt="Select gene_symbol and count of pathogenic variants where count >= 2.", starter_code={"sql": "SELECT gene_symbol, COUNT(*) as variant_count FROM genomic_variants WHERE classification = 'Pathogenic' GROUP BY gene_symbol HAVING COUNT(*) >= 2 ORDER BY variant_count DESC;\n"}, test_cases=[{"description": "Pathogenic variants", "setup_sql": "CREATE TABLE genomic_variants (id INT, gene_symbol TEXT, classification TEXT); INSERT INTO genomic_variants VALUES (1, 'BRCA1', 'Pathogenic'), (2, 'BRCA1', 'Pathogenic'), (3, 'TP53', 'Benign');", "expected_output": [["BRCA1", 2]]}], time_limit_minutes=8),
            AssessmentQuestion(id="bio_deb_1", type="debugging", modality="code_debugging", section="coding", section_title="Section 2: Code Debugging & Threat Defense", title="Debugging: Fix GC-Content Ratio Percentage", prompt="Calculate GC percentage in DNA string: (count('G') + count('C')) / len(seq) * 100. Return rounded to 1 decimal.", starter_code={"python": "def solution(seq):\n    if not seq: return 0.0\n    gc = seq.upper().count('G') + seq.upper().count('C')\n    return round((gc / len(seq)) * 100, 1)\n"}, test_cases=[{"input_data": ["GCGCAT"], "expected_output": 66.7, "description": "4 of 6 are GC = 66.7%"}], time_limit_minutes=8)
        ]
    }

    # 30. ROBOTICS & AUTONOMOUS SYSTEMS ENGINEER
    tracks["robotics_autonomous_engineer"] = {
        "track_id": "robotics_autonomous_engineer", "title": "Robotics & Autonomous Systems Engineer",
        "badge": "Robotics & ROS 2", "description": "ROS 2 DDS middleware, Extended Kalman Filters (EKF), LiDAR point cloud filtering, and path planning (A*/RRT).",
        "skills": ["ROS 2", "C++", "Python", "LiDAR", "Control Systems", "Algorithms", "SQL"],
        "questions": [
            AssessmentQuestion(id="rob_mcq_1", type="mcq", modality="mcq_fundamentals", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="Dead Reckoning and Sensor Fusion", prompt="Why can an autonomous mobile robot not rely solely on wheel encoder dead reckoning for localization over long distances?", options=["A) Wheel encoders drain all battery power.", "B) Wheel slip and terrain unevenness accumulate integration drift unbounded over time, requiring sensor fusion (IMU/LiDAR/SLAM) for correction.", "C) Encoders only report velocity.", "D) Encoders lose calibration every second."], correct_option=1, explanation="Dead reckoning errors compound over distance without external grounding.", time_limit_minutes=2),
            AssessmentQuestion(id="rob_mcq_2", type="mcq", modality="mcq_architecture", section="mcq", section_title="Section 1: Multiple Choice Questions (MCQs)", title="ROS 2 DDS Quality of Service (QoS)", prompt="Why does camera image streaming in ROS 2 typically use 'Best Effort' QoS instead of 'Reliable' QoS?", options=["A) Best effort costs less money.", "B) Dropping an occasional stale image frame is preferable to blocking real-time control loops with TCP-like packet retransmissions.", "C) Best effort encrypts the video.", "D) Reliable QoS is not supported in ROS 2."], correct_option=1, explanation="Real-time robotics prioritizes latest sensory state over retransmission of stale frames.", time_limit_minutes=2),
            AssessmentQuestion(id="rob_dsa_1", type="coding", modality="dsa_algorithms", section="coding", section_title="Section 2: Coding & DSA Challenges", title="Robotics: 2D Euclidean Distance Trajectory Length", prompt="Given a list of 2D waypoints [[x0, y0], [x1, y1], ...], calculate total path length rounded to 2 decimals.", starter_code={"python": "import math\ndef solution(points):\n    dist = 0.0\n    for i in range(len(points) - 1):\n        dx = points[i+1][0] - points[i][0]\n        dy = points[i+1][1] - points[i][1]\n        dist += math.sqrt(dx * dx + dy * dy)\n    return round(dist, 2)\n"}, test_cases=[{"input_data": [[[0, 0], [3, 4], [3, 7]]], "expected_output": 8.0, "description": "5 + 3 = 8.0"}], time_limit_minutes=8),
            AssessmentQuestion(id="rob_sql_1", type="sql", modality="sql_aggregation_analytics", section="sql", section_title="Section 2: Interactive SQL & Data Challenges", title="SQL: Autonomous Vehicle Disengagement Events", prompt="Select vehicle_id and count of disengagements where severity = 'CRITICAL'.", starter_code={"sql": "SELECT vehicle_id, COUNT(*) as critical_events FROM vehicle_disengagements WHERE severity = 'CRITICAL' GROUP BY vehicle_id ORDER BY critical_events DESC;\n"}, test_cases=[{"description": "Critical disengagements", "setup_sql": "CREATE TABLE vehicle_disengagements (id INT, vehicle_id TEXT, severity TEXT); INSERT INTO vehicle_disengagements VALUES (1, 'AV-01', 'CRITICAL'), (2, 'AV-01', 'CRITICAL'), (3, 'AV-02', 'LOW');", "expected_output": [["AV-01", 2]]}], time_limit_minutes=8),
            AssessmentQuestion(id="rob_deb_1", type="debugging", modality="code_debugging", section="coding", section_title="Section 2: Code Debugging & Threat Defense", title="Debugging: Fix Robot Speed Clamping", prompt="Clamp commanded robot speed `v` between `min_v` and `max_v`.", starter_code={"python": "def solution(v, min_v, max_v):\n    return max(min_v, min(v, max_v))\n"}, test_cases=[{"input_data": [12.5, 0.0, 5.0], "expected_output": 5.0, "description": "Clamp speed at max 5.0"}], time_limit_minutes=8)
        ]
    }
...
# Merge all 30 roles together
EXAM_TRACKS_20.update(_build_remaining_15_roles())
EXAM_TRACKS_20.update(_build_additional_10_roles())
EXAM_TRACKS_30: Dict[str, Dict[str, Any]] = EXAM_TRACKS_20
